...
export default function ChauffeurCamionsPage() {
  ...
  const { data: camions, isLoading } = useQuery({
    ...
    queryFn: () => chauffeurService.getMyCamions(),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data: CamionFormValues) => chauffeurService.createCamion(data as CamionCreate),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: CamionFormValues }) =>
      chauffeurService.updateCamion(id, data as CamionUpdate),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => chauffeurService.deleteCamion(id),
    onSuccess: () => {
  ...
  const photoMutation = useMutation({
    mutationFn: ({ camionId, file }: { camionId: string; file: File }) => {
      ...
      fd.append("file", file);
      return chauffeurService.uploadCamionPhoto(camionId, fd);
    },
    onSuccess: () => {
  ...
  const deletePhotoMutation = useMutation({
    mutationFn: ({ camionId, photoId }: { camionId: string; photoId: string }) =>
      chauffeurService.deleteCamionPhoto(camionId, photoId),
    onSuccess: () => {
  ...
  const setMainPhotoMutation = useMutation({
    mutationFn: ({ camionId, photoId }: { camionId: string; photoId: string }) =>
      chauffeurService.setMainPhoto(camionId, photoId),
    onSuccess: () => {
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !viewPhotos) return;
    if (file.size > 5 * 1024 * 1024) {
      toast.error("Max 5 Mo");
      return;
    }
    photoMutation.mutate({ camionId: viewPhotos.id, file });
    e.target.value = "";
  };
  ...
  return (
    <PageAnimation className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500">Gérez vos véhicules</p>
          </div>
        </div>
        <Button onClick={() => { setEditingCamion(null); reset(); setDialogOpen(true); }} className="w-full sm:w-auto min-h-[44px]">
                ...
                >
                  ...
                  <div className="absolute bottom-2 left-2 flex gap-1">
                    ...
                    {camion.photos && camion.photos.length > 0 && (
                      <Badge variant="info" className="text-[10px]">{camion.photos.length} photo(s)</Badge>
                    )}
                  </div>
                </div>
      ...
      <Dialog open={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="Confirmer la suppression" size="sm">
        <p className="text-gray-600 mb-4">Êtes-vous sûr de vouloir supprimer ce camion ?</p>
        <div className="flex flex-col sm:flex-row justify-end gap-2">
        ...
        <div className="space-y-4">
          {viewPhotos?.photos && viewPhotos.photos.length > 0 ? (
            <>
              <div className="relative aspect-video bg-gray-100 rounded-lg overflow-hidden">
                <img src={resolvePhotoUrl(viewPhotos.photos[currentPhotoIndex]?.photo_url) || ""} alt="Photo du camion" className="w-full h-full object-contain" />
                {viewPhotos.photos.length > 1 && (
                  <>
                    <button onClick={() => setCurrentPhotoIndex((i) => (i > 0 ? i - 1 : viewPhotos.photos.length - 1))} className="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-black/50 rounded-full text-white hover:bg-black/70 min-h-[44px] min-w-[44px] flex items-center justify-center">
                      <ChevronLeft className="h-5 w-5" />
                    </button>
                    <button onClick={() => setCurrentPhotoIndex((i) => (i < viewPhotos.photos.length - 1 ? i + 1 : 0))} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-black/50 rounded-full text-white hover:bg-black/70 min-h-[44px] min-w-[44px] flex items-center justify-center">
                      <ChevronRight className="h-5 w-5" />
                    </button>
                  </>
                )}
                <div className="absolute top-2 right-2"><Badge variant="info">{currentPhotoIndex + 1}/{viewPhotos.photos.length}</Badge></div>
              </div>
              ...
              <Button variant="outline" size="sm" onClick={() => { const photo = viewPhotos.photos[currentPhotoIndex]; if (photo && !photo.est_principale) setMainPhotoMutation.mutate({ camionId: viewPhotos.id, photoId: photo.id }); }} disabled={viewPhotos.photos[currentPhotoIndex]?.est_principale}>
                <Star className="h-3.5 w-3.5 mr-1" /> Photo principale
              </Button>
            </>
          ) : (
          ...
          <div className="flex justify-center">
            <Button onClick={() => fileInputRef.current?.click()}>
              <Upload className="h-4 w-4 mr-2" /> Ajouter une photo
            </Button>
          </div>
        </div>
      </Dialog>
